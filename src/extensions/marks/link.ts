...
const extension : IExtension = {
  ...
  marks: [{
    ...
    spec: {
      ...
      inclusive: false,
      parseDOM: [{tag: "a[href]", getAttrs(dom: Node | string) {
  ...
  commands: (schema: Schema, ui: IEditorUI) => {
    return [
      new Command(
        "link", 
        ["Shift-Mod-k", "Shift-Mod-Z"], 
        linkCommand(schema.marks.link, ui.onEditLink)
      )
    ]
  }
};
...
function linkCommand(markType: MarkType, onEditLink: ILinkEditor) {
  ...
  return (state: EditorState, dispatch?: ((tr: Transaction<any>) => void), view?: EditorView) => {
    ...
    if (dispatch) {
      ...
      // show edit ui
      onEditLink(link as ILinkProps)

        .then((result: ILinkEditResult | null) => {
          ...
          if (result) {
            ...
            interface IRange { from: number, to: number }
