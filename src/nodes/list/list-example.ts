...
export function exampleListsAppendTransaction(schema: Schema) {
  return (transactions: Transaction[], oldState: EditorState, newState: EditorState) => {
    if (transactions.some(transaction => transaction.docChanged)) {
      ...
      const exampleLists = findChildrenByType(newState.doc, schema.nodes.ordered_list).filter(
        nodeWithPos => nodeWithPos.node.attrs.number_style === ListNumberStyle.Example,
      );
